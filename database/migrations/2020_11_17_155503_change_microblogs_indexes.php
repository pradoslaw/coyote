...
class ChangeMicroblogsIndexes extends Migration
{
    ...
    public function up()
    {
        $this->schema->table('microblogs', function (Blueprint $table) {
            $table->index([$this->db->raw('is_sponsored DESC'), $this->db->raw('score DESC'), 'parent_id', 'deleted_at']);
            ...
            $table->index([$this->db->raw('is_sponsored DESC'), $this->db->raw('id DESC'), 'parent_id', 'deleted_at']);
