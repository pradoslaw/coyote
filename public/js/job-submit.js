...
$(function () {
     ...
     */
    $('#box-edit-firm').find('input[name="name"]').one('keyup', function () {
        if ($('#firm-id').val() === '') {
            return true;
        }

        $('#modal-firm').modal('show').find('.btn-primary').one('click', function () {
            $.get(_config.firm_partial, {}, function (html) {
