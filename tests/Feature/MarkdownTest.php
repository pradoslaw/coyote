...
class MarkdownTest extends TestCase
{
    ...
    public function testParseUserName()
    {
        ...
        $this->assertRegExp('/<a href=".*">@admin<\/a> lorem ipsum/', $input);

        $input = $this->markdown->parse('@admin');
        $this->assertRegExp('/<a href=".*">@admin<\/a>/', $input);

        $input = $this->markdown->parse('(@admin)');
        $this->assertRegExp('/\(<a href=".*">@admin<\/a>\)/', $input);

        $input = $this->markdown->parse("@admin\n(@admin)");
        $this->assertRegExp("/<a href=\".*\">@admin<\/a>\n\(<a href=\".*\">@admin<\/a>\)/", $input);

        $input = $this->markdown->parse('@admin lorem ipsum `@admin`');
        $this->assertRegExp('/<a href=".*">@admin<\/a> lorem ipsum <code>@admin<\/code>/', $input);
        ...
        $this->assertRegExp('/<a href=".*">@admin<\/a>\'s. lorem ipsum/', $input);

        $input = $this->markdown->parse('@admin @admin');
        $this->assertRegExp('/<a href=".*">@admin<\/a> <a href=".*">@admin<\/a>/', $input);

        $input = $this->markdown->parse("@admin\n@admin");
        $this->assertRegExp("/<a href=\".*\">@admin<\/a>\n<a href=\".*\">@admin<\/a>/", $input);

//        $input = $this->markdown->parse("@admin… foo");
//        $this->assertRegExp("/<strong>@admin…<\/strong> foo/", $input);
        ...
        $this->assertRegExp('/<a href=".*">@admin admiński<\/a> lorem ipsum/', $input);
        ...
        $input = $this->markdown->parse('@admin admiński: lorem ipsum');
        $this->assertRegExp('/<a href=".*">@admin<\/a> admiński: lorem ipsum/', $input);

        $input = $this->markdown->parse('@{admin-admiński} lorem ipsum');
        $this->assertRegExp('/<strong>@admin-admiński<\/strong> lorem ipsum/', $input);
        ...
        $this->assertRegExp('/<strong>@p88<\/strong>.yyy lorem ipsum/', $input);

        $input = $this->markdown->parse('@{p88.yyy}: lorem ipsum');
        $this->assertRegExp('/<a href=".*">@p88.yyy<\/a>: lorem ipsum/', $input);
        ...
        $this->assertRegExp('/support@somedomain.net/', $input);

        $input = $this->markdown->parse('(@somedomain) lorem ipsum');
        $this->assertRegExp('/\(<a href=".*">@somedomain<\/a>\) lorem ipsum/', $input);
    ...
    public function testParseHashTag()
    {
        ...
        $this->assertRegExp('/<a href=".*">#coyote<\/a>/', $input);

        $input = $this->markdown->parse('(#coyote)');
        $this->assertRegExp('/\(<a href=".*">#coyote<\/a>\)/', $input);

        $input = $this->markdown->parse('#coyote #4programmers.net');
        $this->assertRegExp('/<a href=".*">#coyote<\/a> <a href=".*">#4programmers.net<\/a>/', $input);

        $input = $this->markdown->parse("#coyote\n#4programmers.net");
        $this->assertRegExp("/<a href=\".*\">#coyote<\/a>\n<a href=\".*\">#4programmers.net<\/a>/", $input);
    }
}
